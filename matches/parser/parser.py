...
def prepare_parsedlogfile(logfile,replay):
    ...
    for line in logfile:
        if line.find("|")>-1:
            ...
            line=line.replace(", M","").replace(", F","").replace("-*","").replace(", shiny","").replace(", L50","").replace(", L5","").replace("-Striped","").replace("-Trash","").replace("-Sandy","").replace("-Violet","").replace("Indeedee-F","Indeedee").replace("-Super","").replace("-Large","").replace("-Small","").replace("-Blue","").replace("-Orange","").replace("Florges-White","Florges").replace("-Pokeball","").replace("-Elegant","").replace("-Indigo","").replace("-Yellow","").replace("-Bug","").replace("-Dark","").replace("-Dragon","").replace("Silvally-Electric","Silvally").replace("-Fairy","").replace("-Fighting","").replace("-Fire","").replace("-Flying","").replace("-Ghost","").replace("-Grass","").replace("-Ground","").replace("-Ice","").replace("-Normal","").replace("-Poison","").replace("Arceus-Psychic","Arceus").replace("Silvally-Psychic","Silvally").replace("-Rock","").replace("-Steel","").replace("-Water","").replace("-Douse","").replace("-Burn","").replace("-Chill","").replace("-Shock","").replace("Type: ","Type:").replace("-Sensu","").replace("-Pom-Pom","").replace("-Pa'u","").replace("-Totem","").replace("-Resolute","").replace("-Meteor","").replace("Meowstic-F","Meowstic").replace("-East","").replace("fetch’d","fetch'd").replace("-Ruby-Cream","").replace("-Matcha-Cream","").replace("-Mint-Cream","").replace("-Lemon-Cream","").replace("-Salted-Cream","").replace("-Ruby-Swirl","").replace("-Caramel-Swirl","").replace("-Rainbow-Swirl","").replace("Pikachu-Belle","Pikachu").replace("Pikachu-Cosplay","Pikachu").replace("Pikachu-Libre","Pikachu").replace("Pikachu-PhD","Pikachu").replace("Pikachu-Pop-Star","Pikachu").replace("Pikachu-Rock-Star","Pikachu").replace("Pikachu-Starter","Pikachu").replace("Pikachu-Alola","Pikachu").replace("Pikachu-Hoenn","Pikachu").replace("Pikachu-Kalos","Pikachu").replace("Pikachu-Original","Pikachu").replace("Pikachu-Partner","Pikachu").replace("Pikachu-Sinnoh","Pikachu").replace("Pikachu-Unova","Pikachu").replace("Pikachu-World","Pikachu")
            linestoremove=["|","|teampreview","|clearpoke","|upkeep"]
            badlines=["","|start","|player|p1","|player|p2","|player|p1|","|player|p2|","|-notarget","|-clearallboost","|-nothing","|-ohko","|rated","|-swapsideconditions"]
...
def damage_function(line,parsedlogfile,results):
    ...
    if line[3].find("[from]")>-1:
        ...
        elif cause in ['Recoil','item: Life Orb','highjumpkick','recoil','High Jump Kick','Mind Blown','steelbeam','mindblown','Steel Beam','jumpkick','Jump Kick'] or cause.find("Recoil|[of] ")>-1 or cause.find("recoil|[of] ")>-1:
            pokemon['hphealed']+=-damagedone
