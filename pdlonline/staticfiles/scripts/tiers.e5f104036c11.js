$(document).ready(function() {
    ...
    $("#sortby").change(function() {
        ...
        else if (sortingby=="hp-inc"){
            sortMeBy("data-hp", $(".tieritem"), $("#tierlist"), "asc");
        }
        else if (sortingby=="hp-dec"){
            sortMeBy("data-hp", $(".tieritem"), $("#tierlist"), "desc");
        }
    ...
    $("#filteroptions").change(function(){
        var category=$("#filteroptions").val()
        if (category==""){
            $("#filter").html("<option value=''>---------</option>")
        } else if (category=="available"){
            $("#filter").html("<option value=''>---------</option>")
            allavailability=['Available','Unavailable']
            $.each(allavailability, function(i, item) {
                $("#filter").append("<option value='"+item.replace(/ /g,"").replace(/:/g,"").replace(/%/g,"").replace(".","").toLowerCase()+"'>"+item+"</option>")
            })
        } else if (category=="tier"){
            $("#filter").html("<option value=''>---------</option>")
            $.each(tieroptions, function(i, item) {
                $("#filter").append("<option value='"+item.replace(/ /g,"").replace(/:/g,"").replace(/%/g,"").replace(".","").toLowerCase()+"'>"+item+"</option>")
            })
    ...
    $("#addfilter").click(function() {
        ...
        var filter=$("#filter")
        var categorytext=$("#filteroptions option:selected")
        var filtertext=$("#filter option:selected")
        if (category.val() != "" && filter.val() != ""){
            newfilter=category.val()+"-"+filter.val()
            newfilterlabel=categorytext.text()+": "+filtertext.text()
            $("#activefilters").append("<span class='border mr-2 p-1 activefilter' id='"+newfilter+"'>"+newfilterlabel+" <i class='fas fa-times-circle deletefilter'></i></span>")
            filterTiers()
        }
    })

    $("#activefilters").on("click",".deletefilter",function() {
        $(this).parent().remove()
        filterTiers()
    })
})
...
function filterTiers() {
    ...
    if (activefilters.length==0){
        ...
        $(".tieritem").removeClass("d-none")
    } else{
        $("#filterdiv").removeClass("d-none")
        $(".tieritem").removeClass("d-none")
        $.each(activefilters, function(i, item) {
            search=$(item).attr('id')
            $(".tieritem").not("."+search).addClass("d-none")
        })
    }
}
